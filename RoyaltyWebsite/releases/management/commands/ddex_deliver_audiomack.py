"""
Send a release to Audiomack: build DDEX XML and upload to S3 (your bucket or Audiomack's),
and copy cover art + audio into resources/ so the package is complete.
Same as clicking "DDEX delivery for Audiomack" on the Preview & Distribute page.

Run: python manage.py ddex_deliver_audiomack 36306
     python manage.py ddex_deliver_audiomack --upc 8905285301465
"""
...
class Command(BaseCommand):
    help = (
        "Send release to Audiomack: build DDEX ERN 4.3 XML, upload to S3, "
    ...
    def handle(self, *args, **options):
        ...
        if not release_id and not upc:
            self.stderr.write(
                self.style.ERROR("Provide release_id or --upc. Example: python manage.py ddex_deliver_audiomack 36306")
            )
            return
        try:
            if release_id:
        ...
        self.stdout.write(f"Sending release to Audiomack: {release.title} (id={release.id}, upc={release.upc or 'N/A'})")
        success, err, detail = deliver_release_to_audiomack(release)
        if not success:
